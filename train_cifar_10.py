...
def evaluate(model: TransferModel, loader,
             final_report: bool = False):  # eval func we will use to run the model on validation (every epoch) and for testing at the end.
    ...
    with torch.no_grad():  # no weight change--> faster with less memory usage
        loop = tqdm(loader, desc="Evaluation",
                    leave=False) if not final_report else loader  # dont wrap loader in tqdm for  final report

        for images, labels in loop:  # looping over batches
            images, labels = images.to(device), labels.to(
                device)  # send data to gpu (needs to be same device as models)
...
# we take all hyper parameters as arguments (+phase and save name)
def train_phase(model: TransferModel,
                epochs: int, lr: float,
                phase_name: str, save_name: str) -> float:  # and we return best val acc
    criterion = nn.CrossEntropyLoss()  # loss function for training
    optimizer = optim.Adam(model.get_trainable_params(),
                           lr=lr)  # adaptive optimizer updates weights based on the gradient of the loss with respect to each trainable parameter
    ...
    for epoch in range(epochs):
        ...
        for i, (images, labels) in enumerate(loop):  # iterate over each batch
            ...
            optimizer.zero_grad(
                set_to_none=True)  # clears old gradients (none instead of zero)-->less memory and little faster
            outputs = model(images)
            loss = criterion(outputs,
                             labels)  # compute loss using loss function (cross entropy between model output and true labels)
...
def run_training(backbone: str):
    ...
    train_phase(model, epochs=5, lr=1e-4, phase_name=f"{backbone}: lastblock",
                save_name=f"{backbone}_cifar10_lastblock_best.pth")
    ...
    train_phase(model, epochs=15, lr=1e-5, phase_name=f"{backbone}: full",
                save_name=f"{backbone}_cifar10_full_best.pth")
